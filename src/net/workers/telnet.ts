...
const _self = self as DedicatedWorkerContext<WorkerEvent>;

_self.onmessage = (ev: Event) => {
  const { data } = ev as MessageEvent<WorkerEvent>;
  switch (data.type) {
    case WorkerEventType.Init:
      console.log("WORKER INIT CALLED", data.payload);
      _self.postMessage({ type: WorkerEventType.Init, payload: data.payload });
      break;
  }
};
